# Design Distributed Key-Value Store

[← Back to Problems](00-index.md)

---

## 🎯 Problem Statement

Design a distributed key-value store (like DynamoDB, Cassandra, or Redis Cluster) that provides high availability, partition tolerance, and configurable consistency.

**Difficulty**: 🔴 Hard (Tier 1)

---

## 1. Requirements Gathering

### Functional Requirements

1. **Put(key, value)** - Store data
2. **Get(key)** - Retrieve data
3. **Delete(key)** - Remove data
4. **TTL support** - Auto-expiration
5. **Range queries** - Optional, ordered keys
6. **Atomic operations** - CAS, increment

### Non-Functional Requirements

| Aspect | Requirement |
|--------|-------------|
| **Availability** | 99.99% |
| **Latency** | < 10ms p99 |
| **Scalability** | Petabytes of data |
| **Durability** | No data loss |
| **Consistency** | Tunable (eventual to strong) |

---

## 2. Back of Envelope Calculations

```
Data Volume:
- 1 billion keys
- Average value size: 1 KB
- Total: 1 TB data

Traffic:
- 100K read/second
- 10K write/second

Replication:
- 3 replicas per key
- Total storage: 3 TB

Nodes:
- 500 GB per node
- 6 nodes minimum for data
- 3 nodes for coordination
```

---

## 3. Core Concepts

```
┌─────────────────────────────────────────────────────────────────┐
│                   KEY-VALUE STORE CONCEPTS                      │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Data Model:                                                    │
│  ────────────                                                   │
│  key (string) → value (bytes) + metadata                      │
│                                                                 │
│  Metadata:                                                      │
│  • version (vector clock or timestamp)                        │
│  • ttl (expiration time)                                      │
│  • checksum                                                    │
│                                                                 │
│  Storage Format:                                                │
│  ────────────────                                               │
│  ┌────────────────────────────────────────────────────────┐   │
│  │  key_len │  key  │ value_len │ value │ version │ ttl  │   │
│  │   4B     │  var  │    4B     │  var  │   8B    │  8B  │   │
│  └────────────────────────────────────────────────────────┘   │
│                                                                 │
│  Operations:                                                    │
│  ────────────                                                   │
│  PUT(key, value, options)  → success/error                    │
│  GET(key, options)         → value/not_found                  │
│  DELETE(key)               → success/error                    │
│  CAS(key, expected, new)   → success/conflict                 │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 4. High-Level Architecture

```
┌────────────────────────────────────────────────────────────────────────────┐
│                   DISTRIBUTED KEY-VALUE STORE                              │
├────────────────────────────────────────────────────────────────────────────┤
│                                                                            │
│      ┌──────────┐  ┌──────────┐  ┌──────────┐                            │
│      │  Client  │  │  Client  │  │  Client  │                            │
│      └────┬─────┘  └────┬─────┘  └────┬─────┘                            │
│           │             │             │                                    │
│           └─────────────┼─────────────┘                                   │
│                         │                                                  │
│                         ▼                                                  │
│               ┌───────────────────┐                                       │
│               │   Coordinator     │                                       │
│               │   (any node)      │                                       │
│               └─────────┬─────────┘                                       │
│                         │                                                  │
│            ┌────────────┼────────────┐                                    │
│            │            │            │                                    │
│            ▼            ▼            ▼                                    │
│    ┌─────────────┐┌─────────────┐┌─────────────┐                         │
│    │   Node 1    ││   Node 2    ││   Node 3    │                         │
│    │ (Primary)   ││ (Replica)   ││ (Replica)   │                         │
│    └──────┬──────┘└──────┬──────┘└──────┬──────┘                         │
│           │              │              │                                 │
│    ┌──────┴──────┐┌──────┴──────┐┌──────┴──────┐                         │
│    │  Storage    ││  Storage    ││  Storage    │                         │
│    │  Engine     ││  Engine     ││  Engine     │                         │
│    │  (LSM/B+)   ││  (LSM/B+)   ││  (LSM/B+)   │                         │
│    └─────────────┘└─────────────┘└─────────────┘                         │
│                                                                            │
│    ┌────────────────────────────────────────────────────────────────┐    │
│    │                    Hash Ring (Consistent Hashing)              │    │
│    │                                                                 │    │
│    │    ┌───┐     ┌───┐     ┌───┐     ┌───┐     ┌───┐     ┌───┐   │    │
│    │    │N1 │─────│N2 │─────│N3 │─────│N4 │─────│N5 │─────│N6 │   │    │
│    │    └───┘     └───┘     └───┘     └───┘     └───┘     └───┘   │    │
│    │                                                                 │    │
│    └────────────────────────────────────────────────────────────────┘    │
│                                                                            │
└────────────────────────────────────────────────────────────────────────────┘
```

---

## 5. Deep Dive: Partitioning

```
┌─────────────────────────────────────────────────────────────────┐
│                   CONSISTENT HASHING                            │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Hash Ring:                                                     │
│  ────────────                                                   │
│  • Hash keys and nodes to same space (0 to 2^128)             │
│  • Key assigned to first node clockwise                       │
│                                                                 │
│                    ┌───────┐                                   │
│                ┌───│  N1   │───┐                               │
│            ┌───┘   └───────┘   └───┐                           │
│       ┌────┘                       └────┐                      │
│    ┌──┴──┐                           ┌──┴──┐                   │
│    │ N6  │     Keys A, B, C          │ N2  │                   │
│    │     │     hash to this          │     │                   │
│    └──┬──┘     segment → N2          └──┬──┘                   │
│       └────┐                       ┌────┘                      │
│            └───┐   ┌───────┐   ┌───┘                           │
│                └───│  N3   │───┘                               │
│                    └───────┘                                   │
│                                                                 │
│  Virtual Nodes:                                                 │
│  ───────────────                                                │
│  • Each physical node has 100+ virtual nodes                  │
│  • Better distribution                                         │
│  • Easier rebalancing                                          │
│                                                                 │
│  Finding Nodes:                                                 │
│  ───────────────                                                │
│  def get_nodes(key, replication_factor=3):                    │
│      hash = md5(key)                                          │
│      pos = binary_search(ring, hash)                          │
│      nodes = []                                                │
│      while len(nodes) < replication_factor:                   │
│          node = ring[pos % len(ring)]                         │
│          if node.physical not in nodes:                       │
│              nodes.append(node.physical)                      │
│          pos += 1                                             │
│      return nodes                                              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 6. Deep Dive: Replication

```
┌─────────────────────────────────────────────────────────────────┐
│                   REPLICATION STRATEGIES                        │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Replication Factor (N):                                        │
│  ─────────────────────────                                      │
│  • N = 3: Each key stored on 3 nodes                          │
│  • Replicas on consecutive nodes in ring                      │
│                                                                 │
│  Write Path:                                                    │
│  ────────────                                                   │
│  1. Client sends write to coordinator                         │
│  2. Coordinator finds N replicas for key                      │
│  3. Coordinator sends write to all N replicas                 │
│  4. Waits for W acknowledgments (quorum)                      │
│                                                                 │
│  Read Path:                                                     │
│  ───────────                                                    │
│  1. Client sends read to coordinator                          │
│  2. Coordinator sends read to N replicas                      │
│  3. Waits for R responses                                     │
│  4. Returns latest version (highest timestamp/version)        │
│                                                                 │
│  Quorum:                                                        │
│  ─────────                                                      │
│  W + R > N guarantees overlap (read sees latest write)        │
│                                                                 │
│  Configurations:                                                │
│  ────────────────                                               │
│  N=3, W=2, R=2: Strong consistency, good availability        │
│  N=3, W=1, R=1: Eventual consistency, highest availability   │
│  N=3, W=3, R=1: Read-optimized                               │
│  N=3, W=1, R=3: Write-optimized                              │
│                                                                 │
│  ┌────────────────────────────────────────────────────────┐   │
│  │  Write: W=2                    Read: R=2               │   │
│  │                                                         │   │
│  │  ┌───────┐                     ┌───────┐              │   │
│  │  │ Node1 │ ✓ ACK              │ Node1 │ v=5 ✓        │   │
│  │  └───────┘                     └───────┘              │   │
│  │  ┌───────┐                     ┌───────┐              │   │
│  │  │ Node2 │ ✓ ACK              │ Node2 │ v=5 ✓        │   │
│  │  └───────┘                     └───────┘              │   │
│  │  ┌───────┐                     ┌───────┐              │   │
│  │  │ Node3 │ (async)            │ Node3 │ (not needed) │   │
│  │  └───────┘                     └───────┘              │   │
│  │                                                         │   │
│  │  2 ACKs received              Return v=5              │   │
│  │  → Write succeeds             (latest version)        │   │
│  └────────────────────────────────────────────────────────┘   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 7. Deep Dive: Conflict Resolution

```
┌─────────────────────────────────────────────────────────────────┐
│                   CONFLICT RESOLUTION                           │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Problem: Concurrent writes to same key on different nodes    │
│                                                                 │
│  Option 1: Last-Writer-Wins (LWW)                              │
│  ─────────────────────────────────                              │
│  • Attach timestamp to each write                             │
│  • Highest timestamp wins                                      │
│  • Simple but loses data                                       │
│                                                                 │
│  Option 2: Vector Clocks                                        │
│  ────────────────────────────                                   │
│  • Track version per node                                      │
│  • Detect conflicts (concurrent writes)                       │
│  • Application resolves conflicts                             │
│                                                                 │
│  Vector Clock Example:                                          │
│  ──────────────────────                                         │
│  Initial: value=A, vc={N1:1}                                  │
│                                                                 │
│  Client 1 on N1: value=B, vc={N1:2}                          │
│  Client 2 on N2: value=C, vc={N1:1, N2:1}                    │
│                                                                 │
│  Conflict! Neither dominates the other                        │
│  Return both: [(B, {N1:2}), (C, {N1:1, N2:1})]               │
│  Client must merge                                             │
│                                                                 │
│  Option 3: CRDTs                                                │
│  ─────────────────                                              │
│  • Conflict-free data types                                   │
│  • Automatic merge (counters, sets, etc.)                     │
│  • Limited to specific operations                             │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 8. Storage Engine

```
┌─────────────────────────────────────────────────────────────────┐
│                   STORAGE ENGINE (LSM Tree)                     │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Write Path:                                                    │
│  ────────────                                                   │
│  1. Write to Write-Ahead Log (WAL) on disk                    │
│  2. Insert into in-memory MemTable (sorted)                   │
│  3. When MemTable full → flush to SSTable on disk             │
│                                                                 │
│  ┌────────────────────────────────────────────────────────┐   │
│  │                                                         │   │
│  │  Write → WAL → MemTable → SSTable (L0)                │   │
│  │                              │                          │   │
│  │                          Compaction                     │   │
│  │                              ▼                          │   │
│  │                         SSTable (L1)                   │   │
│  │                              │                          │   │
│  │                          Compaction                     │   │
│  │                              ▼                          │   │
│  │                         SSTable (L2)                   │   │
│  │                                                         │   │
│  └────────────────────────────────────────────────────────┘   │
│                                                                 │
│  Read Path:                                                     │
│  ───────────                                                    │
│  1. Check MemTable                                             │
│  2. Check Bloom filters for each SSTable                      │
│  3. Read from SSTable if Bloom filter says "maybe"           │
│                                                                 │
│  Bloom Filters:                                                 │
│  ───────────────                                                │
│  • Probabilistic: no false negatives                          │
│  • Skip SSTables that definitely don't have key              │
│  • Huge read performance improvement                          │
│                                                                 │
│  Compaction:                                                    │
│  ────────────                                                   │
│  • Merge SSTables, remove duplicates/tombstones               │
│  • Leveled: L0 → L1 → L2 (size ratio ~10x)                   │
│  • Background process                                          │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 9. Failure Handling

```
┌─────────────────────────────────────────────────────────────────┐
│                   FAILURE HANDLING                              │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Node Failure Detection:                                        │
│  ─────────────────────────                                      │
│  • Gossip protocol: nodes exchange state                      │
│  • Failure detector: no heartbeat → suspected                 │
│  • Consensus: majority agrees → confirmed dead                │
│                                                                 │
│  Handling Temporary Failures (Hinted Handoff):                 │
│  ───────────────────────────────────────────────                │
│  1. Node N2 is down                                            │
│  2. Write for N2 goes to N3 with "hint"                       │
│  3. N3 stores hint: "deliver to N2 when back"                │
│  4. N2 comes back, N3 sends hinted writes                     │
│                                                                 │
│  ┌────────────────────────────────────────────────────────┐   │
│  │  Normal:          N2 Down:          N2 Recovery:       │   │
│  │                                                         │   │
│  │  ┌───┐           ┌───┐              ┌───┐             │   │
│  │  │N1 │           │N1 │              │N1 │             │   │
│  │  └─┬─┘           └─┬─┘              └───┘             │   │
│  │    │               │                                   │   │
│  │    ▼               ▼                  ┌───┐           │   │
│  │  ┌───┐           ┌───┐ (down)        │N2 │◀──hints   │   │
│  │  │N2 │           │N2 │X              └───┘            │   │
│  │  └─┬─┘           └───┘                  ▲             │   │
│  │    │               │                    │             │   │
│  │    ▼               ▼                  ┌─┴─┐           │   │
│  │  ┌───┐           ┌───┐ + hints       │N3 │           │   │
│  │  │N3 │           │N3 │               └───┘           │   │
│  │  └───┘           └───┘                               │   │
│  └────────────────────────────────────────────────────────┘   │
│                                                                 │
│  Anti-Entropy (Merkle Trees):                                   │
│  ──────────────────────────────                                 │
│  • Build hash tree of data                                     │
│  • Compare trees between replicas                             │
│  • Sync only differing branches                               │
│  • Periodic background repair                                  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 10. Technology Choices

| Component | Technology | Rationale |
|-----------|------------|-----------|
| Storage Engine | RocksDB (LSM) | Write-optimized |
| Coordination | Gossip (Swim) | Decentralized |
| Serialization | Protocol Buffers | Efficient |
| Networking | gRPC | High performance |
| Consistency | Quorum-based | Tunable |
| Hashing | MurmurHash | Fast, uniform |

---

## 11. Key Takeaways

```
┌─────────────────────────────────────────────────────────────────┐
│                   KEY TAKEAWAYS                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  1. CONSISTENT HASHING                                          │
│     Minimal data movement on scaling                           │
│                                                                 │
│  2. TUNABLE CONSISTENCY                                         │
│     W + R > N for strong consistency                           │
│                                                                 │
│  3. LSM TREE                                                    │
│     Write-optimized storage engine                             │
│                                                                 │
│  4. HINTED HANDOFF                                              │
│     Handle temporary failures gracefully                       │
│                                                                 │
│  5. VECTOR CLOCKS                                               │
│     Detect and resolve conflicts                               │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 12. References

- [14-distributed-patterns.md](../fundamentals/14-distributed-patterns.md) - Consensus
- [04-consistent-hashing.md](../fundamentals/04-consistent-hashing.md) - Partitioning
- [11-databases.md](../fundamentals/11-databases.md) - Storage engines

---

[← Back to Problems](00-index.md) | [Next: News Feed Ranking →](20-news-feed-ranking.md)
