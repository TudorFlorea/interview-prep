# Design News Feed Ranking

[← Back to Problems](00-index.md)

---

## 🎯 Problem Statement

Design a personalized news feed ranking system (like Facebook/Twitter feed) that uses machine learning to order content based on user engagement signals and relevance.

**Difficulty**: 🔴 Hard (Tier 1)

---

## 1. Requirements Gathering

### Functional Requirements

1. **Generate personalized feed** - Tailored to each user
2. **Rank by relevance** - Not just chronological
3. **Real-time updates** - New content appears quickly
4. **Support content types** - Posts, ads, recommendations
5. **Explainability** - Why am I seeing this?
6. **Diversity** - Avoid filter bubbles

### Non-Functional Requirements

| Aspect | Requirement |
|--------|-------------|
| **Latency** | < 200ms for feed |
| **Freshness** | New posts within 1 minute |
| **Personalization** | Per-user ranking |
| **Scale** | 1 billion users |
| **A/B Testing** | Support experiments |

---

## 2. Back of Envelope Calculations

```
Users:
- 1 billion users
- 100 million DAU
- Average: 500 followees per user

Feed Generation:
- 100M feed requests/day
- 1,000 requests/second average
- Peak: 10,000 requests/second

Candidates:
- 500 followees × 10 recent posts = 5,000 candidates
- Rank down to 50 for initial feed

Model Inference:
- 5,000 candidates × 10,000 requests/sec = 50M inferences/sec
- Need efficient model + caching
```

---

## 3. Core Entities

```sql
-- Posts (content items)
CREATE TABLE posts (
    post_id UUID PRIMARY KEY,
    author_id UUID NOT NULL,
    content TEXT,
    media_urls JSON,
    post_type ENUM('text', 'image', 'video', 'link'),
    created_at TIMESTAMP,
    
    INDEX idx_author_time (author_id, created_at DESC)
);

-- User features (for ML)
CREATE TABLE user_features (
    user_id UUID PRIMARY KEY,
    embedding VECTOR(128),  -- User interest embedding
    topic_affinities JSON,  -- {politics: 0.3, sports: 0.8}
    engagement_history JSON,
    last_active TIMESTAMP,
    updated_at TIMESTAMP
);

-- Post features (for ML)
CREATE TABLE post_features (
    post_id UUID PRIMARY KEY,
    embedding VECTOR(128),  -- Content embedding
    topics JSON,
    engagement_stats JSON,  -- {likes: 100, comments: 20}
    author_features JSON,
    quality_score FLOAT,
    updated_at TIMESTAMP
);

-- Engagement events (for training)
CREATE TABLE engagements (
    event_id UUID PRIMARY KEY,
    user_id UUID,
    post_id UUID,
    action ENUM('view', 'like', 'comment', 'share', 'hide', 'report'),
    dwell_time_ms INT,
    created_at TIMESTAMP,
    
    INDEX idx_user_time (user_id, created_at)
);
```

---

## 4. High-Level Architecture

```
┌────────────────────────────────────────────────────────────────────────────┐
│                   NEWS FEED RANKING ARCHITECTURE                           │
├────────────────────────────────────────────────────────────────────────────┤
│                                                                            │
│                           ┌─────────────┐                                 │
│                           │    User     │                                 │
│                           └──────┬──────┘                                 │
│                                  │                                        │
│                                  ▼                                        │
│                         ┌─────────────────┐                              │
│                         │  Feed Service   │                              │
│                         └────────┬────────┘                              │
│                                  │                                        │
│         ┌────────────────────────┼────────────────────────┐              │
│         │                        │                        │               │
│         ▼                        ▼                        ▼               │
│  ┌─────────────┐         ┌─────────────┐         ┌─────────────┐        │
│  │  Candidate  │         │   Ranking   │         │   Blending  │        │
│  │ Generation  │────────▶│    Model    │────────▶│  & Mixing   │        │
│  │             │         │             │         │             │        │
│  └──────┬──────┘         └──────┬──────┘         └──────┬──────┘        │
│         │                       │                       │                │
│         │                       │                       ▼                │
│         │                       │              ┌─────────────┐          │
│         │                       │              │    Feed     │          │
│         │                       │              │   Response  │          │
│         │                       │              └─────────────┘          │
│         │                       │                                        │
│  ┌──────┴──────┐         ┌──────┴──────┐                               │
│  │             │         │             │                                │
│  ▼             ▼         ▼             ▼                                │
│ ┌────┐      ┌─────┐   ┌──────┐    ┌─────────┐                         │
│ │Post│      │User │   │Feature│   │  Model  │                         │
│ │Index│     │Graph│   │ Store │   │ Serving │                         │
│ └────┘      └─────┘   └──────┘    └─────────┘                         │
│                                                                          │
│  ──────────────────── Offline Pipeline ─────────────────────           │
│                                                                          │
│  ┌─────────────┐   ┌─────────────┐   ┌─────────────┐                  │
│  │ Engagement  │──▶│   Feature   │──▶│   Model     │                  │
│  │    Logs     │   │ Engineering │   │  Training   │                  │
│  └─────────────┘   └─────────────┘   └─────────────┘                  │
│                                                                          │
└────────────────────────────────────────────────────────────────────────────┘
```

---

## 5. Deep Dive: Candidate Generation

```
┌─────────────────────────────────────────────────────────────────┐
│                   CANDIDATE GENERATION                          │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Goal: Reduce billions of posts to thousands of candidates    │
│                                                                 │
│  Sources:                                                       │
│  ─────────                                                      │
│  1. Following Feed                                              │
│     • Posts from users you follow                              │
│     • Recent posts (last 24-48 hours)                         │
│                                                                 │
│  2. Friend-of-Friend                                            │
│     • Posts liked/shared by friends                           │
│     • Viral content from network                              │
│                                                                 │
│  3. Interest-based                                              │
│     • Posts matching user's topic interests                   │
│     • Similar to previously engaged content                   │
│                                                                 │
│  4. Trending                                                    │
│     • Globally or locally trending posts                      │
│                                                                 │
│  Retrieval Methods:                                             │
│  ───────────────────                                            │
│  ┌────────────────────────────────────────────────────────┐   │
│  │                                                         │   │
│  │  1. Fan-out on Read:                                   │   │
│  │     Query posts from each followee                     │   │
│  │     Merge and sort                                     │   │
│  │     O(followees × posts_per_user)                     │   │
│  │                                                         │   │
│  │  2. Embedding Similarity (ANN):                        │   │
│  │     user_embedding → nearest post_embeddings           │   │
│  │     Using FAISS, ScaNN, or Milvus                     │   │
│  │     O(log N) approximate search                       │   │
│  │                                                         │   │
│  │  3. Inverted Index:                                    │   │
│  │     topic → posts mapping                             │   │
│  │     User topics → candidate posts                     │   │
│  │                                                         │   │
│  └────────────────────────────────────────────────────────┘   │
│                                                                 │
│  Candidate Pool:                                                │
│  ────────────────                                               │
│  • 1,000 - 10,000 candidates per user                         │
│  • Diverse sources to avoid echo chamber                      │
│  • Pre-filtered for relevance                                 │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 6. Deep Dive: Ranking Model

```
┌─────────────────────────────────────────────────────────────────┐
│                   RANKING MODEL                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Multi-Objective Ranking:                                       │
│  ─────────────────────────                                      │
│  Predict multiple engagement types:                            │
│  • P(like | user, post)                                       │
│  • P(comment | user, post)                                    │
│  • P(share | user, post)                                      │
│  • P(dwell_time > 30s | user, post)                          │
│  • P(hide | user, post) - negative signal                     │
│                                                                 │
│  Final Score:                                                   │
│  ─────────────                                                  │
│  score = w1 × P(like) + w2 × P(comment) + w3 × P(share)      │
│          + w4 × P(dwell) - w5 × P(hide)                       │
│                                                                 │
│  Features:                                                      │
│  ──────────                                                     │
│  ┌────────────────────────────────────────────────────────┐   │
│  │                                                         │   │
│  │  User Features:           Post Features:               │   │
│  │  • User embedding         • Post embedding             │   │
│  │  • Topic interests        • Topics                     │   │
│  │  • Activity level         • Post age                   │   │
│  │  • Friend count           • Engagement count           │   │
│  │  • Demographics           • Author quality score       │   │
│  │  • Device/context         • Media type                 │   │
│  │                                                         │   │
│  │  Cross Features:                                       │   │
│  │  • User-author affinity                               │   │
│  │  • User-topic match                                   │   │
│  │  • Historical engagement                              │   │
│  │  • Social connection strength                         │   │
│  │                                                         │   │
│  └────────────────────────────────────────────────────────┘   │
│                                                                 │
│  Model Architecture:                                            │
│  ────────────────────                                           │
│  Two-Tower + Cross Network:                                    │
│                                                                 │
│  User Tower    Post Tower                                      │
│      │             │                                           │
│      └──────┬──────┘                                           │
│             │                                                   │
│         Dot Product (for retrieval)                            │
│             │                                                   │
│      Cross Network (for ranking)                               │
│             │                                                   │
│         Final Score                                            │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 7. Feature Engineering

```
┌─────────────────────────────────────────────────────────────────┐
│                   FEATURE ENGINEERING                           │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Real-time Features:                                            │
│  ────────────────────                                           │
│  • Post age (seconds since creation)                          │
│  • Current engagement velocity                                 │
│  • User's recent activity                                      │
│  • Time of day                                                 │
│                                                                 │
│  Near-real-time (updated every minute):                        │
│  ────────────────────────────────────────                       │
│  • Post engagement counts                                      │
│  • Trending scores                                             │
│  • Author recent activity                                      │
│                                                                 │
│  Batch Features (updated daily):                                │
│  ─────────────────────────────────                              │
│  • User embeddings                                             │
│  • Topic affinities                                            │
│  • Long-term engagement patterns                               │
│                                                                 │
│  Feature Store:                                                 │
│  ───────────────                                                │
│  ┌────────────────────────────────────────────────────────┐   │
│  │                                                         │   │
│  │  ┌─────────────┐    ┌─────────────┐                   │   │
│  │  │   Batch     │    │  Streaming  │                   │   │
│  │  │   Pipeline  │    │  Pipeline   │                   │   │
│  │  └──────┬──────┘    └──────┬──────┘                   │   │
│  │         │                  │                           │   │
│  │         └────────┬─────────┘                          │   │
│  │                  ▼                                     │   │
│  │           ┌─────────────┐                             │   │
│  │           │   Feature   │                             │   │
│  │           │    Store    │                             │   │
│  │           │   (Redis)   │                             │   │
│  │           └──────┬──────┘                             │   │
│  │                  │                                     │   │
│  │                  ▼                                     │   │
│  │           ┌─────────────┐                             │   │
│  │           │   Serving   │                             │   │
│  │           │    Layer    │                             │   │
│  │           └─────────────┘                             │   │
│  │                                                         │   │
│  └────────────────────────────────────────────────────────┘   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 8. Feed Diversity & Quality

```
┌─────────────────────────────────────────────────────────────────┐
│                   DIVERSITY & QUALITY                           │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Problem: Pure relevance ranking creates echo chambers         │
│                                                                 │
│  Diversity Techniques:                                          │
│  ──────────────────────                                         │
│  1. MMR (Maximal Marginal Relevance)                           │
│     score = λ × relevance - (1-λ) × max_similarity_to_selected│
│     Balances relevance with diversity                          │
│                                                                 │
│  2. Topic Quotas                                                │
│     • Max 30% from any single topic                           │
│     • Inject content from underrepresented topics             │
│                                                                 │
│  3. Source Diversity                                            │
│     • Limit posts from same author                            │
│     • Mix different content types                             │
│                                                                 │
│  Quality Signals:                                               │
│  ─────────────────                                              │
│  • Author credibility score                                    │
│  • Content quality classifier                                  │
│  • Engagement authenticity                                     │
│  • Misinformation detection                                    │
│                                                                 │
│  Blending Logic:                                                │
│  ────────────────                                               │
│  def blend_feed(ranked_posts, ads, recommendations):           │
│      feed = []                                                  │
│      for i, post in enumerate(ranked_posts):                  │
│          feed.append(post)                                     │
│          if i % 5 == 4:  # Every 5th position                 │
│              feed.append(ads.pop(0))                          │
│          if i % 10 == 9:  # Every 10th position              │
│              feed.append(recommendations.pop(0))              │
│      return feed                                               │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 9. Model Training Pipeline

```
┌─────────────────────────────────────────────────────────────────┐
│                   TRAINING PIPELINE                             │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Training Data:                                                 │
│  ───────────────                                                │
│  • (user, post, action, label) tuples                         │
│  • Positive: user engaged with post                           │
│  • Negative: user saw but didn't engage                       │
│                                                                 │
│  Negative Sampling:                                             │
│  ───────────────────                                            │
│  • Random negatives (not seen)                                │
│  • Hard negatives (seen but ignored)                          │
│  • In-batch negatives (efficient)                             │
│                                                                 │
│  Pipeline:                                                      │
│  ──────────                                                     │
│  ┌────────────────────────────────────────────────────────┐   │
│  │                                                         │   │
│  │  Engagement Logs → Feature Join → Training Data        │   │
│  │         │                              │                │   │
│  │         ▼                              ▼                │   │
│  │    Label Events                  Feature Store         │   │
│  │         │                              │                │   │
│  │         └──────────────┬───────────────┘               │   │
│  │                        ▼                                │   │
│  │                 ┌────────────┐                         │   │
│  │                 │   Train    │                         │   │
│  │                 │   Model    │                         │   │
│  │                 └─────┬──────┘                         │   │
│  │                       │                                 │   │
│  │                       ▼                                 │   │
│  │                 ┌────────────┐                         │   │
│  │                 │  Evaluate  │                         │   │
│  │                 │   (A/B)    │                         │   │
│  │                 └─────┬──────┘                         │   │
│  │                       │                                 │   │
│  │                       ▼                                 │   │
│  │                 ┌────────────┐                         │   │
│  │                 │   Deploy   │                         │   │
│  │                 └────────────┘                         │   │
│  │                                                         │   │
│  └────────────────────────────────────────────────────────┘   │
│                                                                 │
│  Retraining:                                                    │
│  ────────────                                                   │
│  • Daily incremental updates                                   │
│  • Weekly full retraining                                      │
│  • A/B test before rollout                                     │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 10. Technology Choices

| Component | Technology | Rationale |
|-----------|------------|-----------|
| Candidate Retrieval | FAISS/ScaNN | Fast ANN search |
| Feature Store | Redis/Feast | Low latency |
| Model Serving | TensorFlow Serving | Scalable inference |
| Training | PyTorch/TensorFlow | Flexible ML |
| A/B Testing | Custom | Statistical rigor |
| Stream Processing | Flink | Real-time features |

---

## 11. Key Takeaways

```
┌─────────────────────────────────────────────────────────────────┐
│                   KEY TAKEAWAYS                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  1. TWO-STAGE RANKING                                           │
│     Candidate generation → Ranking model                       │
│                                                                 │
│  2. MULTI-OBJECTIVE                                             │
│     Predict multiple engagement types                          │
│                                                                 │
│  3. FEATURE ENGINEERING                                         │
│     Real-time + batch features                                 │
│                                                                 │
│  4. DIVERSITY                                                   │
│     Avoid filter bubbles with MMR                              │
│                                                                 │
│  5. CONTINUOUS LEARNING                                         │
│     Retrain on fresh engagement data                           │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 12. References

- [05-caching.md](../fundamentals/05-caching.md) - Feature caching
- [09-message-queues.md](../fundamentals/09-message-queues.md) - Event streaming
- [22-batch-processing.md](../fundamentals/22-batch-processing.md) - ML pipelines

---

[← Back to Problems](00-index.md) | [Next: Amazon Locker →](21-amazon-locker.md)
