# Distributed System Patterns

[← Back to Fundamentals](00-index.md)

---

## Overview

Distributed systems require special patterns to handle coordination, failure, and consistency across multiple nodes. This guide covers essential patterns including leader election, consensus, distributed transactions, and the saga pattern.

---

## 👑 Leader Election

### Why Leader Election?

```
┌─────────────────────────────────────────────────────────────────┐
│                    LEADER ELECTION NEED                         │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Problem: Some tasks need a single coordinator                  │
│                                                                 │
│  Examples:                                                      │
│  - Database primary (one writer)                                │
│  - Cron job runner (run once, not N times)                      │
│  - Partition assignment (consistent decisions)                  │
│  - Lock management (one source of truth)                        │
│                                                                 │
│  Without leader:              With leader:                      │
│  ────────────────             ─────────────                     │
│  ┌───┐ ┌───┐ ┌───┐           ┌───┐ ┌───┐ ┌───┐                │
│  │ A │ │ B │ │ C │           │ A │ │★B★│ │ C │                │
│  └─┬─┘ └─┬─┘ └─┬─┘           └─┬─┘ └─┬─┘ └─┬─┘                │
│    │     │     │               │     │     │                   │
│    ▼     ▼     ▼               │     ▼     │                   │
│  Write Write Write             └────►B◄────┘                   │
│  (conflict!)                    (coordinated)                   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### Election Approaches

#### 1. Bully Algorithm

```
┌─────────────────────────────────────────────────────────────────┐
│                    BULLY ALGORITHM                              │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Rule: Highest ID wins                                          │
│                                                                 │
│  Nodes: A(1), B(2), C(3) — C is leader                         │
│                                                                 │
│  C fails:                                                       │
│  1. B notices, sends ELECTION to higher IDs (C)                │
│  2. B gets no response (C is dead)                             │
│  3. B declares itself leader, sends COORDINATOR msg            │
│                                                                 │
│  C recovers:                                                    │
│  1. C sends ELECTION to higher IDs (none)                      │
│  2. C declares itself leader immediately                       │
│  3. C sends COORDINATOR to all                                 │
│                                                                 │
│  ✅ Simple to understand                                        │
│  ❌ Many messages                                               │
│  ❌ Assumes IDs are well-ordered                                │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

#### 2. Using Zookeeper/etcd

```
┌─────────────────────────────────────────────────────────────────┐
│                    ZOOKEEPER LEADER ELECTION                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Each node creates ephemeral sequential znode:                  │
│                                                                 │
│  /election/                                                     │
│  ├── candidate_0000001  (Node A)                               │
│  ├── candidate_0000002  (Node B)                               │
│  └── candidate_0000003  (Node C)                               │
│                                                                 │
│  Rule: Lowest sequence number is leader                         │
│                                                                 │
│  Leader (A) crashes:                                            │
│  1. Ephemeral node deleted automatically                        │
│  2. B watches its predecessor (was A)                          │
│  3. B notified A's node gone                                   │
│  4. B becomes leader (now lowest)                              │
│                                                                 │
│  ✅ Reliable (Zookeeper handles complexity)                     │
│  ✅ Automatic failover (ephemeral nodes)                        │
│  ✅ Watch mechanism (efficient notifications)                   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

#### 3. Using Database Locks

```sql
-- Simple leader election with database

-- Create election table
CREATE TABLE leader_election (
    lock_name VARCHAR(100) PRIMARY KEY,
    leader_id VARCHAR(100) NOT NULL,
    acquired_at TIMESTAMP NOT NULL,
    expires_at TIMESTAMP NOT NULL
);

-- Try to become leader (atomic)
INSERT INTO leader_election (lock_name, leader_id, acquired_at, expires_at)
VALUES ('scheduler', 'node-1', NOW(), NOW() + INTERVAL '30 seconds')
ON CONFLICT (lock_name) DO UPDATE
SET leader_id = EXCLUDED.leader_id,
    acquired_at = EXCLUDED.acquired_at,
    expires_at = EXCLUDED.expires_at
WHERE leader_election.expires_at < NOW();  -- Only if expired

-- Renew leadership (heartbeat)
UPDATE leader_election
SET expires_at = NOW() + INTERVAL '30 seconds'
WHERE lock_name = 'scheduler' AND leader_id = 'node-1';
```

---

## 🤝 Consensus Algorithms

### The Consensus Problem

```
┌─────────────────────────────────────────────────────────────────┐
│                    CONSENSUS PROBLEM                            │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Goal: All nodes agree on a single value/decision               │
│                                                                 │
│  Properties required:                                           │
│  1. Agreement: All non-faulty nodes decide same value           │
│  2. Validity: The decided value was proposed by some node       │
│  3. Termination: All non-faulty nodes eventually decide         │
│                                                                 │
│  Challenges:                                                    │
│  - Nodes can crash                                              │
│  - Messages can be lost/delayed                                 │
│  - Network can partition                                        │
│                                                                 │
│  Used for:                                                      │
│  - Leader election                                              │
│  - Distributed locks                                            │
│  - Replicated state machines                                    │
│  - Transaction commit                                           │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### Raft Consensus

```
┌─────────────────────────────────────────────────────────────────┐
│                    RAFT OVERVIEW                                │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Three roles:                                                   │
│  - Leader: Handles all client requests                         │
│  - Follower: Replicates leader's log                           │
│  - Candidate: Trying to become leader                           │
│                                                                 │
│  Normal Operation:                                              │
│  ─────────────────                                              │
│            Client                                               │
│               │                                                 │
│               ▼                                                 │
│          ┌────────┐                                             │
│          │ Leader │ ──────────────────────────┐                 │
│          └────────┘                           │                 │
│               │                               │                 │
│          Replicate                       Replicate              │
│               │                               │                 │
│               ▼                               ▼                 │
│          ┌──────────┐                   ┌──────────┐           │
│          │ Follower │                   │ Follower │           │
│          └──────────┘                   └──────────┘           │
│                                                                 │
│  1. Client sends request to leader                              │
│  2. Leader appends to log                                       │
│  3. Leader replicates to followers                              │
│  4. Majority ack → committed                                    │
│  5. Leader responds to client                                   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### Raft Leader Election

```
┌─────────────────────────────────────────────────────────────────┐
│                    RAFT ELECTION                                │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  1. Follower times out (no heartbeat from leader)               │
│  2. Follower becomes Candidate                                  │
│  3. Candidate increments term, votes for self                   │
│  4. Candidate requests votes from all nodes                     │
│  5. Nodes vote (one vote per term)                              │
│  6. Majority votes → Candidate becomes Leader                   │
│                                                                 │
│  Timeline:                                                      │
│  ────────────────────────────────────────────►                  │
│                                                                 │
│  Term 1:  [─────── Leader A ───────]                            │
│                                    │                            │
│                                    A crashes                    │
│                                    │                            │
│  Term 2:  [Election timeout]──────►│                            │
│           B: RequestVote(term=2)   │                            │
│           C: Vote for B            │                            │
│           B wins!                  │                            │
│  Term 2:           [────── Leader B ──────]                     │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### Paxos (Brief Overview)

```
┌─────────────────────────────────────────────────────────────────┐
│                    PAXOS PHASES                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Phase 1: Prepare                                               │
│  ─────────────────                                              │
│  Proposer → Acceptors: "I want to propose (n)"                  │
│  Acceptors → Proposer: "OK, here's highest I've seen"           │
│                                                                 │
│  Phase 2: Accept                                                │
│  ────────────────                                               │
│  Proposer → Acceptors: "Accept value V with proposal n"         │
│  Acceptors → Proposer: "Accepted" (if n still highest)          │
│                                                                 │
│  Majority accepts → Value chosen!                               │
│                                                                 │
│  Paxos vs Raft:                                                 │
│  ─────────────────────────────────────────                      │
│  Paxos: More theoretical, harder to understand                  │
│  Raft: Designed for understandability, same guarantees          │
│                                                                 │
│  In practice: Use Raft or existing implementations              │
│  (etcd, Consul, CockroachDB all use Raft)                       │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 💰 Distributed Transactions

### Two-Phase Commit (2PC)

```
┌─────────────────────────────────────────────────────────────────┐
│                    TWO-PHASE COMMIT                             │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Phase 1: Prepare (Voting)                                      │
│  ──────────────────────────                                     │
│                                                                 │
│       Coordinator                                               │
│           │                                                     │
│       Prepare?────────┬───────────┐                             │
│           │           │           │                             │
│           ▼           ▼           ▼                             │
│       ┌───────┐   ┌───────┐   ┌───────┐                        │
│       │Node A │   │Node B │   │Node C │                        │
│       └───┬───┘   └───┬───┘   └───┬───┘                        │
│           │           │           │                             │
│         Yes         Yes         Yes                             │
│           │           │           │                             │
│           └───────────┼───────────┘                             │
│                       ▼                                         │
│                 All voted Yes!                                  │
│                                                                 │
│  Phase 2: Commit (Decision)                                     │
│  ───────────────────────────                                    │
│                                                                 │
│       Coordinator                                               │
│           │                                                     │
│       COMMIT──────────┬───────────┐                             │
│           │           │           │                             │
│           ▼           ▼           ▼                             │
│       ┌───────┐   ┌───────┐   ┌───────┐                        │
│       │Node A │   │Node B │   │Node C │                        │
│       │COMMIT │   │COMMIT │   │COMMIT │                        │
│       └───────┘   └───────┘   └───────┘                        │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 2PC Problems

```
┌─────────────────────────────────────────────────────────────────┐
│                    2PC BLOCKING PROBLEM                         │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Scenario: Coordinator crashes after sending Prepare            │
│                                                                 │
│       Coordinator ✗                                             │
│           │                                                     │
│       Prepare?────────┐                                         │
│           │           │                                         │
│           ✗           ▼                                         │
│                   ┌───────┐                                     │
│                   │Node A │  "I voted Yes..."                   │
│                   │Locked!│  "Do I commit or abort?"            │
│                   │  ???  │  "Coordinator is dead!"             │
│                   └───────┘                                     │
│                                                                 │
│  Node A is BLOCKED:                                             │
│  - Can't commit (no decision received)                          │
│  - Can't abort (maybe others committed)                         │
│  - Must hold locks until coordinator recovers                   │
│                                                                 │
│  This is why 2PC is rarely used in practice for                 │
│  systems requiring high availability                            │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### Three-Phase Commit (3PC)

```
Adds a Pre-Commit phase to avoid blocking:

Phase 1: Prepare    → Participants vote
Phase 2: Pre-Commit → Coordinator broadcasts intent  
Phase 3: Commit     → Actual commit

If coordinator dies after Pre-Commit, participants
can coordinate among themselves to complete.

Still not commonly used due to complexity.
Better: Use Saga pattern or avoid distributed transactions.
```

---

## 🎭 Saga Pattern

### Overview

```
┌─────────────────────────────────────────────────────────────────┐
│                    SAGA PATTERN                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Alternative to distributed transactions                        │
│  Break transaction into local transactions + compensations      │
│                                                                 │
│  Example: Book a trip (flight + hotel + car)                    │
│  ─────────────────────────────────────────────                  │
│                                                                 │
│  Forward transactions:                                          │
│  T1: Book flight                                                │
│  T2: Book hotel                                                 │
│  T3: Book car                                                   │
│                                                                 │
│  Compensating transactions (undo):                              │
│  C1: Cancel flight                                              │
│  C2: Cancel hotel                                               │
│  C3: Cancel car                                                 │
│                                                                 │
│  If T3 fails:                                                   │
│  Execute C2, then C1 (reverse order)                           │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### Saga Execution Patterns

#### Choreography (Event-Driven)

```
┌─────────────────────────────────────────────────────────────────┐
│                    CHOREOGRAPHY SAGA                            │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Each service listens for events and triggers next step         │
│                                                                 │
│  ┌─────────────┐     FlightBooked     ┌─────────────┐          │
│  │   Flight    │ ───────────────────► │    Hotel    │          │
│  │   Service   │                      │   Service   │          │
│  └─────────────┘                      └──────┬──────┘          │
│        ▲                                     │                  │
│        │                              HotelBooked               │
│  OrderCreated                                │                  │
│        │                                     ▼                  │
│  ┌─────────────┐                      ┌─────────────┐          │
│  │   Order     │                      │     Car     │          │
│  │   Service   │ ◄─────────────────── │   Service   │          │
│  └─────────────┘     SagaComplete     └─────────────┘          │
│                                                                 │
│  ✅ Loose coupling                                              │
│  ✅ Simple for small sagas                                      │
│  ❌ Hard to understand flow                                     │
│  ❌ Cyclic dependencies possible                                │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

#### Orchestration (Centralized)

```
┌─────────────────────────────────────────────────────────────────┐
│                    ORCHESTRATION SAGA                           │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Central orchestrator controls the saga                         │
│                                                                 │
│                    ┌──────────────┐                             │
│                    │ Orchestrator │                             │
│                    └──────┬───────┘                             │
│                           │                                     │
│         ┌─────────────────┼─────────────────┐                   │
│         │                 │                 │                   │
│         ▼                 ▼                 ▼                   │
│  ┌─────────────┐   ┌─────────────┐   ┌─────────────┐           │
│  │   Flight    │   │    Hotel    │   │     Car     │           │
│  │   Service   │   │   Service   │   │   Service   │           │
│  └─────────────┘   └─────────────┘   └─────────────┘           │
│                                                                 │
│  Orchestrator:                                                  │
│  1. Call Flight.Book()                                          │
│  2. If success → Call Hotel.Book()                              │
│  3. If success → Call Car.Book()                                │
│  4. If any fails → Call compensations                           │
│                                                                 │
│  ✅ Clear flow, easy to understand                              │
│  ✅ Easy to add/modify steps                                    │
│  ❌ Orchestrator can be bottleneck                              │
│  ❌ Tighter coupling to orchestrator                            │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### Saga State Machine

```csharp
public class BookTripSaga
{
    private SagaState _state = SagaState.Started;
    private string _flightId;
    private string _hotelId;
    private string _carId;
    
    public async Task Execute()
    {
        try
        {
            // Step 1: Book Flight
            _state = SagaState.BookingFlight;
            _flightId = await _flightService.Book(request);
            
            // Step 2: Book Hotel
            _state = SagaState.BookingHotel;
            _hotelId = await _hotelService.Book(request);
            
            // Step 3: Book Car
            _state = SagaState.BookingCar;
            _carId = await _carService.Book(request);
            
            _state = SagaState.Completed;
        }
        catch (Exception ex)
        {
            await Compensate();
        }
    }
    
    private async Task Compensate()
    {
        _state = SagaState.Compensating;
        
        // Reverse order!
        if (_carId != null)
            await _carService.Cancel(_carId);
            
        if (_hotelId != null)
            await _hotelService.Cancel(_hotelId);
            
        if (_flightId != null)
            await _flightService.Cancel(_flightId);
            
        _state = SagaState.Compensated;
    }
}
```

---

## 🔌 Circuit Breaker Pattern

### The Problem

```
┌─────────────────────────────────────────────────────────────────┐
│                    CASCADING FAILURE                            │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Service B is slow/failing:                                     │
│                                                                 │
│  Service A ──req──► Service B (slow)                            │
│     │                  │                                        │
│     │ threads         waiting...                                │
│     │ waiting         waiting...                                │
│     │                  │                                        │
│     ▼                  ▼                                        │
│  A's thread pool     Eventually timeout                         │
│  exhausted!                                                     │
│                                                                 │
│  Now A can't handle ANY requests                                │
│  Failure cascades to A's callers!                               │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### Circuit Breaker States

```
┌─────────────────────────────────────────────────────────────────┐
│                    CIRCUIT BREAKER STATES                       │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│        Success                    Failure threshold             │
│           │                             │                       │
│           ▼                             ▼                       │
│      ┌─────────┐              ┌─────────────┐                  │
│      │ CLOSED  │─────────────►│    OPEN     │                  │
│      │         │  Too many    │             │                  │
│      │(normal) │  failures    │(fail fast)  │                  │
│      └─────────┘              └──────┬──────┘                  │
│           ▲                          │                          │
│           │                      Timeout                        │
│           │                          │                          │
│           │                          ▼                          │
│           │                   ┌─────────────┐                  │
│           │                   │  HALF-OPEN  │                  │
│           └───────────────────│             │                  │
│                Success        │ (testing)   │                  │
│                               └─────────────┘                  │
│                                      │                          │
│                                   Failure                       │
│                                      │                          │
│                                      ▼                          │
│                                    OPEN                         │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### Implementation

```csharp
public class CircuitBreaker
{
    private State _state = State.Closed;
    private int _failureCount = 0;
    private DateTime _openedAt;
    
    private const int FailureThreshold = 5;
    private const int TimeoutSeconds = 30;
    
    public async Task<T> Execute<T>(Func<Task<T>> action)
    {
        if (_state == State.Open)
        {
            if (DateTime.UtcNow - _openedAt > TimeSpan.FromSeconds(TimeoutSeconds))
            {
                _state = State.HalfOpen;
            }
            else
            {
                throw new CircuitOpenException("Circuit is open");
            }
        }
        
        try
        {
            var result = await action();
            OnSuccess();
            return result;
        }
        catch (Exception)
        {
            OnFailure();
            throw;
        }
    }
    
    private void OnSuccess()
    {
        _failureCount = 0;
        _state = State.Closed;
    }
    
    private void OnFailure()
    {
        _failureCount++;
        if (_failureCount >= FailureThreshold)
        {
            _state = State.Open;
            _openedAt = DateTime.UtcNow;
        }
    }
}
```

---

## 🔁 Retry Patterns

### Exponential Backoff

```
┌─────────────────────────────────────────────────────────────────┐
│                    EXPONENTIAL BACKOFF                          │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Attempt 1: Wait 1 second                                       │
│  Attempt 2: Wait 2 seconds                                      │
│  Attempt 3: Wait 4 seconds                                      │
│  Attempt 4: Wait 8 seconds                                      │
│  Attempt 5: Wait 16 seconds (or max)                            │
│                                                                 │
│  With jitter (randomization):                                   │
│  Wait = min(cap, base * 2^attempt) + random(0, 1000ms)          │
│                                                                 │
│  Why jitter? Prevent thundering herd:                           │
│  Without jitter: 1000 clients all retry at exactly 2 seconds    │
│  With jitter: Clients spread out their retries                  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### Retry Implementation

```csharp
public async Task<T> RetryWithBackoff<T>(
    Func<Task<T>> action,
    int maxRetries = 3,
    int baseDelayMs = 1000)
{
    var attempt = 0;
    
    while (true)
    {
        try
        {
            return await action();
        }
        catch (Exception ex) when (IsTransient(ex) && attempt < maxRetries)
        {
            attempt++;
            
            // Exponential backoff with jitter
            var delay = Math.Min(30000, baseDelayMs * Math.Pow(2, attempt));
            var jitter = Random.Shared.Next(0, 1000);
            
            await Task.Delay((int)delay + jitter);
        }
    }
}

private bool IsTransient(Exception ex)
{
    return ex is TimeoutException 
        || ex is HttpRequestException
        || (ex is HttpResponseException hre && hre.StatusCode >= 500);
}
```

---

## 🎯 Idempotency

### Why Idempotency Matters

```
┌─────────────────────────────────────────────────────────────────┐
│                    IDEMPOTENCY NEED                             │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Network is unreliable. Requests can be:                        │
│  - Lost                                                         │
│  - Duplicated                                                   │
│  - Received but response lost                                   │
│                                                                 │
│  Without idempotency:                                           │
│  Client: Pay $100                                               │
│  Server: Deducts $100, sends OK                                 │
│  Network: Loses response                                        │
│  Client: Timeout! Retry: Pay $100                               │
│  Server: Deducts $100 again!                                    │
│  Result: Customer charged $200                                  │
│                                                                 │
│  With idempotency:                                              │
│  Client: Pay $100 (idempotency_key=abc123)                      │
│  Server: Deducts $100, stores key, sends OK                     │
│  Network: Loses response                                        │
│  Client: Retry: Pay $100 (idempotency_key=abc123)               │
│  Server: Sees key already used, returns cached response         │
│  Result: Customer charged $100 correctly                        │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### Implementing Idempotency

```csharp
public class PaymentService
{
    public async Task<PaymentResult> ProcessPayment(PaymentRequest request)
    {
        // Check if we've seen this request before
        var existingResult = await _cache.GetAsync($"payment:{request.IdempotencyKey}");
        if (existingResult != null)
        {
            return existingResult;  // Return cached result
        }
        
        // Use database transaction with unique constraint
        try
        {
            await using var transaction = await _db.BeginTransactionAsync();
            
            // Insert idempotency record (fails if duplicate)
            await _db.InsertAsync(new IdempotencyRecord
            {
                Key = request.IdempotencyKey,
                CreatedAt = DateTime.UtcNow
            });
            
            // Process payment
            var result = await ProcessPaymentInternal(request);
            
            // Store result for future lookups
            await _cache.SetAsync(
                $"payment:{request.IdempotencyKey}",
                result,
                TimeSpan.FromHours(24)
            );
            
            await transaction.CommitAsync();
            return result;
        }
        catch (UniqueConstraintException)
        {
            // Duplicate request - return cached or in-progress
            return await WaitForResult(request.IdempotencyKey);
        }
    }
}
```

---

## ✅ Key Takeaways

1. **Leader election** - Use Zookeeper/etcd, don't build your own
2. **Consensus is hard** - Use Raft implementations (etcd, Consul)
3. **Avoid 2PC** - Use Saga pattern for distributed transactions
4. **Saga compensations** - Design every action with an undo
5. **Circuit breakers** - Prevent cascading failures
6. **Always retry with backoff** - And add jitter!
7. **Make operations idempotent** - Use idempotency keys

---

## 📚 Related Topics

- [CAP Theorem](13-cap-theorem.md) - Consistency trade-offs
- [Message Queues](09-message-queues.md) - Async communication
- [Fault Tolerance](20-fault-tolerance.md) - Handling failures
- [Databases](06-databases.md) - Transaction fundamentals
