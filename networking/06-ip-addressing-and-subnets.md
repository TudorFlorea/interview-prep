...
│   Everything else (allocated by IANA → RIRs → ISPs)                     │
│                                                                          │
...
│                                                                          │
│   CIDR Notation:  192.168.1.0/24  (24 = number of network bits)         │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│            SUBNETTING: Divide 192.168.1.0/24 into 4 subnets             │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│   Original: 192.168.1.0/24 (256 addresses)                              │
│   Need: 4 subnets                                                        │
│   Bits to borrow: 2 (2^2 = 4 subnets)                                   │
│   New prefix: /24 + 2 = /26                                              │
│                                                                          │
│   ┌──────────────────────────────────────────────────────────────────┐  │
...
```
/24 → 256 addresses (block size)
Split into:
  2 subnets → /25 (128 each)
  4 subnets → /26 (64 each)
  8 subnets → /27 (32 each)
  16 subnets → /28 (16 each)
...
│                                                                          │
│   128 bits total = 2^128 ≈ 340 undecillion addresses                    │
│                                                                          │
│   Prefix notation: 2001:db8::/32                                        │
│                    └── First 32 bits are network                        │
│                                                                          │
│   Special addresses:                                                     │
...
│   └─────────────────────────────────────────────────────────────────┘   │
│                                                                          │
│   Reserved addresses per subnet (AWS):                                   │
│   .0 = Network, .1 = VPC router, .2 = DNS, .3 = Future, .255 = Broadcast│
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
### Exercise 1: Calculate Subnet Details 🟢
...
- 45 / 16 = 2.8, so network starts at 2 × 16 = 32
- Network: 172.16.32.0
...
┌────────────────────────────────────────────────────────────────────┐
│ Availability Zone A (10.0.0.0 - 10.0.15.255)                      │
├────────────────────────────────────────────────────────────────────┤
│ Public Subnet A:   10.0.0.0/24    (256 addresses, 254 usable)     │
│ Private App A:     10.0.1.0/23    (512 addresses, 510 usable)     │
│ Private DB A:      10.0.3.0/26    (64 addresses, 62 usable)       │
└────────────────────────────────────────────────────────────────────┘

┌────────────────────────────────────────────────────────────────────┐
│ Availability Zone B (10.0.16.0 - 10.0.31.255)                     │
├────────────────────────────────────────────────────────────────────┤
│ Public Subnet B:   10.0.16.0/24   (256 addresses, 254 usable)     │
│ Private App B:     10.0.17.0/23   (512 addresses, 510 usable)     │
│ Private DB B:      10.0.19.0/26   (64 addresses, 62 usable)       │
└────────────────────────────────────────────────────────────────────┘

┌────────────────────────────────────────────────────────────────────┐
│ Availability Zone C (10.0.32.0 - 10.0.47.255)                     │
├────────────────────────────────────────────────────────────────────┤
│ Public Subnet C:   10.0.32.0/24   (256 addresses, 254 usable)     │
│ Private App C:     10.0.33.0/23   (512 addresses, 510 usable)     │
│ Private DB C:      10.0.35.0/26   (64 addresses, 62 usable)       │
└────────────────────────────────────────────────────────────────────┘
...
- 📍 **IPv4 = 32 bits**: Dotted decimal, ~4.3 billion addresses, running out
- 🏠 **Private ranges**: 10.x.x.x, 172.16-31.x.x, 192.168.x.x (not routed publicly)
- ➗ **CIDR notation**: /24 = 256 addresses, /16 = 65,536 addresses
- 🔢 **Subnetting**: Borrow bits from host portion to create smaller networks
- 🌐 **IPv6 = 128 bits**: Hex notation, essentially unlimited addresses
- ☁️ **Cloud VPCs**: Plan CIDR blocks for growth, separate public/private
